...
static unsigned long neigh_rand_retry(struct neighbour* neigh) {
	int b = NEIGH_VAR(neigh->parms, RETRANS_RAND_BACKOFF_MS);
	if (b) {
		return get_random_u32_below(b);
	}
	return 0;
}
...
static struct neigh_sysctl_table {
	...
	.neigh_vars = {
		...
		NEIGH_SYSCTL_ZERO_INTMAX_ENTRY(RETRANS_RAND_BACKOFF_MS, "retrans_rand_backoff_ms"),
			  ...
			  proc_handler *handler)
{
	...
	if (handler) {
		...
		t->neigh_vars[NEIGH_VAR_RETRANS_RAND_BACKOFF_MS].proc_handler = handler;
