		...
		unsigned long long addr)
{
	...
	if (fault_type == INTR_REMAP) {
		pr_info("[INTR-REMAP] Request device [%02x:%02x.%d] fault index 0x%llx [fault reason 0x%02x] %s\n",
	...
	if (pasid == IOMMU_PASID_INVALID)
		pr_info("[%s NO_PASID] Request device [%02x:%02x.%d] fault addr 0x%llx [fault reason 0x%02x] %s\n",
		       type ? "DMA Read" : "DMA Write",
		       source_id >> 8, PCI_SLOT(source_id & 0xFF),
		       PCI_FUNC(source_id & 0xFF), addr,
		       fault_reason, reason);
	else
		pr_info("[%s PASID 0x%x] Request device [%02x:%02x.%d] fault addr 0x%llx [fault reason 0x%02x] %s\n",
...
irqreturn_t dmar_fault(int irq, void *dev_id)
{
	...
	if (fault_status && __ratelimit(&rs))
		pr_info("DRHD: handling fault status reg %x\n", fault_status);
